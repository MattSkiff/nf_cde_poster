...
Figure \@ref(fig:cde) shows kernel CDE [@bashtannyk2001bandwidth] [@rosenblatt1969conditional] using the faithful geyser data via the `hdrcde` R package, demonstrating an improvement in UQ by using CDE instead of intervals. Early CDE methods included Kernel-CDE, Mixture Density Networks [@carney2005predicting] and discretisation of the target variable via class probability estimators [@frank2009conditional] (Figure \@ref(fig:qr)). Modern methods for CDE include Random Forest-CDE [@pospisil2018rfcde] and Bottleneck Conditional Density Estimation, a variation on Conditional-VAEs [@shu2017bottleneck]. 
