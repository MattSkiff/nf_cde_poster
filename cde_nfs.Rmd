...
Normalising flows are sequences of invertible, differentiable, transformations (bijections) upon a base probability distribution (e.g. a simple gaussian) to approximate the true density, which may be skewed, multi-modal or complex (even discontinuous) [@kobyzev2020normalizing]. NFs were proposed as a density estimation procedure, then for use as approximate posteriors in variational inference. They came to prominence for efficiency and expressiveness in both sampling (generative direction) and density evaluation (normalising direction). NFs fit alongside VAEs and GANs as recent DL generative models, however, VAEs and GANs are not efficient for density evaluation. 
...
Normalising flows are trained by maximising log-likelihood via SGD, or minimising KL-divergence when variational inference is used. Three properties are important for NFs [@kobyzev2020normalizing]:
...
Practical NF models often use **coupling** functions (NICE, RealNVP, Neural Spline Flows, Glow). Coupling functions split the input into disjoint partions, applying an arbitrarily complex conditioning function (e.g. an invertible NN) to one partition. Other models use coupling functions where the conditioner is **autoregressive** (MAFs, IAFs, NAFs). Recent developments include continuous NFs using neural ODEs (e.g. FFJORD) and research on manifold learning [@kobyzev2020normalizing].

## NFs for CDE

Normalising flows have been applied to conditional class probability estimation, conditional image generation and multivariate time series prediction. Work directly on CDE with NFs is limited. This includes Bayesian NFs, which have a framework for placing priors over CDE estimators using BNNs with variational inference [@trippe2018conditional]. CDE using MAFs and Real NVP by conditioning each term in the chain rule of probability with the inclusion of $y$ at every layer was proposed in 2017 [@papamakarios2017masked]. This was explored further while introducing noise regularisation for CDE in 2019 [@rothfuss2019noise]. Conditional NFs for structured prediction tasks have also been developed [@winkler2019learning]. Progress in using NFs for CDE has been limited by the following factors:

* Computational difficulty of scaling NFs to large data sets. For example, the newer continuous NF models are restricted to usage on small image benchmark sets [@grathwohl2018ffjord].
* DL focus is often in areas of traditional strength, e.g. image data. 

Below is a 2D example of using NFs to create CDEs of rainfall and soil moisture (data from NIWA, over 10 months). We scale the data, train the flow, and condition on two levels of rainfall (-1,1) to show the conditional densities of soil moisture. Note that the separate estimates have different modality, which could reflect a differing seasonal effect. Other climatological variables, such as radiance, air temperature, wind and spatio-temporal factors would all be other sources of variation not considered here. In this case, the marginals are simulated well, but the joint is less well approximated.

```{r nf_cde_diag,out.extra='style="float:right; padding:10px"',fig.cap="\\label{fig:nf_cde}Traing NFs for CDE with climatological data",out.width="100%",echo=FALSE}
# All defaults
knitr::include_graphics('figures\\poster_nf_cde_diag.jpg')
```
