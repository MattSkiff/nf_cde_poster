...
Conditional density estimation is a *task* for which there are methods in statistics, machine learning and deep learning. It is a generalisation of conditional mean estimation often performed with MLR. However, in CDE, instead of predicting a point estimate and generated a confidence or credible interval, we are instead concerned with predicting the full conditional density of the data for a given query point, thus representing an improved form of **uncertainty quantification**. Figure \ref{fig:figs} demonstrates kernel CDE [@bashtannyk2001bandwidth] [@rosenblatt1969conditional] using the old faithful dataset via the `hdrcde` R package. Early methods included KCDE and discretisation of the target variable via class probability estimators [@frank2009conditional] (Figure \ref{fig:qr}). Modern methods for CDE include Random Forest-CDE [@pospisil2018rfcde], Mixture Density Networks [@carney2005predicting] and GMMs [@gilardi2002conditional]. 
...
Normalising flows are usually trained by minimising the KL-divergence via standard optimisation techniques, like SGD. The chronology of NF
...
This intersection is a natural extension of progress achieved in the separate fields of CDE and NF research. Some research already exists, such as CDE with Bayesian NFs [@],[@],[@], as a section describing CDE using MAFs [@] but progress has been limited by the following factors:

* Computational difficulty of scaling NFs to large data sets. For example, the latest continuous normalising flows models are restricted to usage on compartively small benchmark sets
...
[^1]:  Poster produced via the `posterdown` package. The code to reproduce this poster is at **https://github.com/MattSkiff/nf_cde_poster**. </font>
